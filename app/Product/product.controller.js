...
class ProductController {
    ...
    async createProduct(req, res) {
        ...
        if (!foundedCategory) throw new ErrorHandler({
            statusCode: StatusCodes.ERROR_NOT_FOUND,
            httpCode: 404,
            result: 'Failed to find category'
        })
        ...
        const result = await this.productService.createProduct({
            ...
            recipe,
            fee,
            category: {_id: foundedCategory._id, categoryName: foundedCategory.name}
        })
    ...
    async deleteProduct(req, res) {
        ...
        if (!result) throw new ErrorHandler({statusCode: StatusCodes.ERROR_NOT_FOUND, httpCode: 404})
            ...
            await Promise.all([
                this.categoryService.removeProductFromCategory({
    ...
    async toggleDisable(req, res) {
        ...
        await this.productValidator.isValidMongoose(productId)
        await this.productService.toggleDisable({productId})

        return ResponseHandler.send({
            res,
            httpCode: 200,
            statusCode: StatusCodes.RESPONSE_SUCCESSFUL,
            result: 'Product disability toggled'
        }, {new: true})
    }
}
